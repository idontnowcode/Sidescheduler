...
await ln.setViewportSize({ width: 1440, height: 860 })
await ln.waitForTimeout(400)
// 업무 탭으로 전환
await ln.locator('.rp-tab', { hasText: '업무' }).click()
await ln.waitForTimeout(700)
const rp = await ln.locator('.rp-panel').boundingBox()
const ed = await ln.locator('.ql-editor').boundingBox()
console.log('오른쪽 열:', Math.round(rp.width) + 'x' + Math.round(rp.height), '/ 본문 높이:', Math.round(ed.height))
await ln.screenshot({ path: join(out, 'wo-side.png') })
// 편집 폼도
await ln.locator('.wo-edit-btn', { hasText: '편집' }).click()
await ln.waitForTimeout(700)
await ln.screenshot({ path: join(out, 'wo-side-edit.png'), clip: { x: rp.x - 4, y: rp.y, width: rp.width + 8, height: Math.min(760, rp.height) } })
