...
function todayAt(timeStr: string, now: Date): number {
...
export default function Timeline() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await add({
      ...
      start_at: todayAt(form.start, now),
      end_at: todayAt(form.end, now),
      color: form.color
    })
  ...
  return (
    <div className="px-3 py-3">
            ...
            {COLORS.map((c) => (
              <button
                ...
                className={`w-5 h-5 rounded-full transition-transform ${
                  form.color === c ? 'scale-125 ring-2 ring-offset-1 ring-blue-400' : 'hover:scale-110'
                }`}
      ...
      {/* Event list */}
      {events.length === 0 ? (
        <p className="text-[11px] text-gray-300 py-2 text-center">오늘 일정이 없습니다</p>
      ) : (
